...
public class MenuPrincipalDashboardController {
    ...
    private void configurarPermisos() {
        ...
        configurarBoton(
                btnAdmin,
                RolPermisos.puedeAccederAdmin()
        );

        configurarBoton(
                btnBodega,
                RolPermisos.puedeAccederBodega()
        );

        configurarBoton(
                btnCajero,
                RolPermisos.puedeAccederCajero()
        );

        configurarBoton(
                btnGestionUsuarios,
                RolPermisos.puedeGestionarUsuarios()
        );

        configurarBoton(
                btnCambiarPassword,
                RolPermisos.puedeCambiarPassword()
        );
    }
    ...
    @FXML
    private void handleInicio(Event event) {
        // Ya estamos en el menú principal.
    }

    @FXML
    private void handleAdmin(Event event) {

        if (!RolPermisos.puedeAccederAdmin()) {
        ...
        abrirVista(
                "/org/libreria/view/AdminDashboardView.fxml"
        );
    }

    @FXML
    private void handleBodega(Event event) {

        if (!RolPermisos.puedeAccederBodega()) {
    ...
    @FXML
    private void handleCajero(Event event) {

        if (!RolPermisos.puedeAccederCajero()) {
    ...
    @FXML
    private void handleGestionUsuarios(Event event) {
        ...
        abrirVista(
                "/org/libreria/view/GestionUsuariosView.fxml"
        );
    }

    @FXML
    private void handleCambiarPassword(Event event) {
        ...
        abrirVista(
                "/org/libreria/view/CambiarPasswordView.fxml"
        );
    }
